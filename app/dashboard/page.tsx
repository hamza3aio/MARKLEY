...
export default async function DashboardPage() {
  ...
  const [{ data: logs }, classIds] = await Promise.all([
    admin.from("activity_logs").select("action,created_at").eq("actor_id", viewer.id).order("created_at", { ascending: false }).limit(5),
    myClassIds(admin, viewer),
  ]);
  let upcoming: { id: string; title: string; due_date: string; class_name: string }[] = [];
  let unread = 0;
  let myPoints = 0;
  if (classIds.length) {
    const [{ data: classes }, { data: asgs }, { data: allPts }, { count: nc }] = await Promise.all([
      admin.from("classes").select("id,name").in("id", classIds),
      admin.from("assignments").select("id,title,due_date,class_id").in("class_id", classIds).is("deleted_at", null).eq("status", "published").not("due_date", "is", null).gte("due_date", new Date().toISOString()).order("due_date", { ascending: true }).limit(5),
      admin.from("points").select("points").eq("user_id", viewer.id).limit(2000),
      admin.from("notifications").select("id", { count: "exact", head: true }).eq("user_id", viewer.id).is("read_at", null),
    ]);
    unread = nc ?? 0;
